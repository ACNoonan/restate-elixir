...
- [docs/demo-4-fan-out.md](./docs/demo-4-fan-out.md) — high-concurrency fan-out throughput. **20,000 in-flight Restate invocations on a single elixir-handler pod, +1 MB peak memory over baseline, 2,489 leaves/sec sustained.** The BEAM has so much headroom that Restate's ingest queue is the bottleneck; per-process generational GC keeps memory delta sublinear in concurrency.
